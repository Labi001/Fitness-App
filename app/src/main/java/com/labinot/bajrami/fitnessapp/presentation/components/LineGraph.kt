...
import com.labinot.bajrami.fitnessapp.presentation.util.changeLocalDateToGraphDate
...
fun LineGraph(
    ...
    textStyle: TextStyle = MaterialTheme.typography.bodySmall.copy(color = MaterialTheme.colorScheme.onBackground)
)
{
    val textMeasurer = rememberTextMeasurer()
    val dataPointValue = bodyPartValues.asReversed().map { it.value }
    val dates = bodyPartValues.asReversed().map { it.date.changeLocalDateToGraphDate() }

    val highestValue = dataPointValue.maxOrNull() ?: 0f
    val lowestValue = dataPointValue.minOrNull() ?: 0f
    val noOfParts = 3
  ...
  Canvas(modifier = modifier)
  {
      ...
      val points = calculatePoints(dataPointValue, graphWidth ,graphHeight)
      ...
      valuesList.forEachIndexed { index,value ->
          ...
          drawText(
              ...
              text = "$value",
              style = textStyle,
              topLeft = Offset(x = xPosition,y= (graph80PercentHeight / noOfParts) * index),
          )
